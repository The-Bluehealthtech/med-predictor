...
# Script final de nettoyage complet - Nettoie tout en une fois
# Usage: ./scripts/cleanup-everything.sh

set -e

echo "🧹 NETTOYAGE COMPLET ET FINAL"
echo "============================="

# Créer une sauvegarde
BACKUP_DIR="storage/backups/cleanup-everything-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
REPORT_FILE="storage/logs/cleanup-everything-report-$(date +%Y%m%d_%H%M%S).md"

cat > "$REPORT_FILE" << EOF
# Rapport de Nettoyage Complet - Med-Predictor

**Date:** $(date)
**Script:** cleanup-everything.sh
...
# Résumé final
echo ""
echo "🎉 NETTOYAGE COMPLET ET FINAL TERMINÉ !"
echo "======================================="
