...
# Script final complet de nettoyage - Nettoie tout en une fois
# Usage: ./scripts/complete-final-cleanup.sh
...
BACKUP_DIR="storage/backups/complete-final-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
REPORT_FILE="storage/logs/complete-final-cleanup-report-$(date +%Y%m%d_%H%M%S).md"

cat > "$REPORT_FILE" << EOF
# Rapport de Nettoyage Final Complet - Med-Predictor
