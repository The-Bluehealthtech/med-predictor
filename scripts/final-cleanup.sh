...
# Script final de nettoyage - Simple et efficace
# Usage: ./scripts/final-cleanup.sh
...
BACKUP_DIR="storage/backups/final-cleanup-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
cp -r app/ "$BACKUP_DIR/"
...
for controller in "${CONTROLLERS[@]}"; do
    ...
    # Nettoyer dans les contrôleurs principaux
    ...
    # Nettoyer dans les contrôleurs API (si le répertoire existe)
    if [ -d "app/Http/Controllers/Api" ]; then
        for controller_file in app/Http/Controllers/Api/*.php; do
            if [ -f "$controller_file" ] && grep -q "$controller" "$controller_file"; then
                if clean_file "$controller_file" "$controller"; then
                    controller_cleaned=$((controller_cleaned + 1))
                fi
...
rm -f routes/*.tmp app/Http/Controllers/*.tmp app/Http/Controllers/Api/*.tmp 2>/dev/null || true
...
for file in routes/*.php; do
    if [ -f "$file" ]; then
        if php -l "$file" >/dev/null 2>&1; then
            echo "  ✅ routes/$(basename "$file"): OK"
        else
            echo "  ❌ routes/$(basename "$file"): Erreur"
...
for file in app/Http/Controllers/*.php; do
    if [ -f "$file" ]; then
        if php -l "$file" >/dev/null 2>&1; then
            echo "  ✅ app/Http/Controllers/$(basename "$file"): OK"
        else
            echo "  ❌ app/Http/Controllers/$(basename "$file"): Erreur"
            syntax_errors=$((syntax_errors + 1))
        fi
    fi
done

# Vérifier les contrôleurs API
if [ -d "app/Http/Controllers/Api" ]; then
    for file in app/Http/Controllers/Api/*.php; do
        if [ -f "$file" ]; then
            if php -l "$file" >/dev/null 2>&1; then
                echo "  ✅ app/Http/Controllers/Api/$(basename "$file"): OK"
            else
                echo "  ❌ app/Http/Controllers/Api/$(basename "$file"): Erreur"
                syntax_errors=$((syntax_errors + 1))
            fi
        fi
    done
...
REPORT_FILE="storage/logs/final-cleanup-report-$(date +%Y%m%d_%H%M%S).md"

cat > "$REPORT_FILE" << EOF
# Rapport de Nettoyage Final - Med-Predictor

**Date:** $(date)
**Script:** final-cleanup.sh
