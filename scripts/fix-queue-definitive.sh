...
echo "🔧 Correction définitive de la queue Laravel..."
echo "=============================================="
...
echo "🔧 Correction définitive de la queue..."

# Mettre la queue en mode sync (plus sûr que redis)
...
# Désactiver l'authentification Redis
kubectl set env deployment/med-predictor-app -n med-predictor REDIS_AUTH=false

if [ $? -eq 0 ]; then
    echo "✅ Variable REDIS_AUTH mise à jour"
else
    echo "❌ Échec de la mise à jour de REDIS_AUTH"
    exit 1
fi
