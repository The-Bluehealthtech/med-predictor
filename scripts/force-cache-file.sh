...
echo "🔧 Forçage de la Configuration Cache en Fichier..."
echo "=================================================="
...
# Créer une configuration qui force le cache en fichier
echo "🔧 Création de la configuration forcée..."

cat << 'EOF' > /tmp/.env.force
...
echo "📝 Copie de la configuration forcée..."
docker cp /tmp/.env.force fit-php:/var/www/html/.env
...
docker exec fit-php php artisan config:show cache.store

# Nettoyer le fichier temporaire
rm -f /tmp/.env.force

echo ""
echo "🎉 Configuration forcée terminée !"
echo "🌐 Testez maintenant: http://localhost:8080"
echo "📊 L'application devrait fonctionner avec le cache forcé en fichier"
