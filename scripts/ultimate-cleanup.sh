...
# Script ultime de nettoyage - Nettoie tout en une fois
# Usage: ./scripts/ultimate-cleanup.sh

set -e

echo "🧹 NETTOYAGE ULTIME DU PROJET"
echo "=============================="

# Créer une sauvegarde
BACKUP_DIR="storage/backups/ultimate-cleanup-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
REPORT_FILE="storage/logs/ultimate-cleanup-report-$(date +%Y%m%d_%H%M%S).md"

cat > "$REPORT_FILE" << EOF
# Rapport de Nettoyage Ultime - Med-Predictor

**Date:** $(date)
**Script:** ultimate-cleanup.sh
